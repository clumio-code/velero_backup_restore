...
def lambda_handler(events, context):
    ...
    try:
        desc_volume = client_ebs.describe_volumes(
            Filters=[
                {"Name": "tag:source_backup_id", "Values": [source_backup_id]},
            ],
        )
    ...
    if len(desc_volume.get("Volumes", [])) == 0:
        return {
            "status": 404,
            "msg": f"no volumes found {desc_volume}",
            "manifest": {},
                ...
                TagSpecifications=[
                    {
                        ...
                        "Tags": [
                            {"Key": "original_volume", "Value": source_volume_id},
                            {"Key": "original_backup_id", "Value": source_backup_id},
                        ],
                    },
                ],
                DryRun=False,
