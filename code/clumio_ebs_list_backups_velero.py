...
def lambda_handler(events: EventsTypeDef, context: LambdaContext) -> dict[str, Any]:
    ...
    source_volume_id = velero_manifest_dict.get("spec", {}).get(
        "providerVolumeID", None
    )
    ...
    try:
        ...
        ebs_volume_resp = common.invoke_sdk_method(
            function=client.aws_ebs_volumes_v1.list_aws_ebs_volumes,
            filter=json.dumps(list_filter),
        )
        if not ebs_volume_resp[1].embedded.items:
            logger.error(
                f"No volume found for {source_volume_id} in account {source_account}"
            )
            # If the volume is not found, return a 40X error.
            return {
                "status": 401,
                "msg": f"No volume found for {source_volume_id} in account {source_account}",
            }

        # Get the backup record of the volume.
        api_filter["volume_id"] = {"$eq": ebs_volume_resp[1].embedded.items[0].p_id}
        raw_backup_records = [
            backup_record
            for backup_record in common.paginated_iterator(
                list_method=client.backup_aws_ebs_volumes_v2.list_backup_aws_ebs_volumes,
    ...
    for backup in raw_backup_records:
        if (
            backup.account_native_id == source_account
            and backup.aws_region == source_region
        ):
            backup_record = {
    ...
    inputs = {"record": [], "velero_manifest": velero_manifest_dict}
    if not backup_records:
        return {
            "status": 404,
            "msg": f"volume {source_volume_id} not found in backup records",
            "inputs": inputs,
        }
