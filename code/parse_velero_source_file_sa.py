...
def lambda_handler(events, context):
    ...
    inputs = {"records": [], "day_offset": None}
    ...
    try:
        ...
        if len(file_content) > 0:
            ...
            inputs = {"records": bigger_list, "day_offset": days_diff}
            logger.info(f"File {source_uri}, found {len(bigger_list)} records.")
            return {"status": 200, "inputs": inputs, "msg": "parsed velero file"}
        else:
            inputs = {"records": file_content, "day_offset": None}
            logger.info(f"File {source_uri}, no records found.")
            return {"status": 207, "inputs": inputs, "msg": "no records"}

    except Exception as e:
        logger.error(f"Error reading file {source_uri}: {e}")
